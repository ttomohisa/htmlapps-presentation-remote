...
foreach ($dependency in $dependencies) {
  ...
  $assetsToEmbed = @($dependency.assets)
  if ($dependency.PSObject.Properties.Name -contains "assetDirectories") {
    foreach ($directory in @($dependency.assetDirectories)) {
      $directoryPath = Get-RelativeAssetPath $package.Root ([string]$directory.path)
      if (-not (Test-Path -PathType Container $directoryPath)) { throw "Dependency asset directory not found: $directoryPath" }
      $keyPrefix = Get-SafeId ([string]$directory.keyPrefix)
      $extensions = @($directory.includeExtensions | ForEach-Object { ([string]$_).ToLowerInvariant() })
      $directoryMime = ""
      if ($directory.PSObject.Properties.Name -contains "mime") { $directoryMime = [string]$directory.mime }
      $directoryCompression = "auto"
      if ($directory.PSObject.Properties.Name -contains "compression") { $directoryCompression = [string]$directory.compression }
      $packageRootFull = [System.IO.Path]::GetFullPath($package.Root).TrimEnd([char[]]@([char]92, [char]47))
      foreach ($file in Get-ChildItem -File -Path $directoryPath | Sort-Object Name) {
        if ($extensions.Count -gt 0 -and $extensions -notcontains $file.Extension.ToLowerInvariant()) { continue }
        $safeLeaf = ($file.Name.ToLowerInvariant() -replace '[^a-z0-9._-]', '-')
        $generatedKey = Get-SafeId ("$keyPrefix-$safeLeaf")
        $fileFull = [System.IO.Path]::GetFullPath($file.FullName)
        $relativePath = $fileFull.Substring($packageRootFull.Length + 1).Replace([char]92, [char]47)
        $assetsToEmbed += [pscustomobject]@{
          key = $generatedKey
          path = $relativePath
          mime = $directoryMime
          stripSourceMapComment = $false
  ...
  foreach ($asset in $assetsToEmbed) {
